...
def split_dataset(train_set, test_set):
    ...
    with open(outputdir, "wb") as output_file:
        np.savez_compressed(output_file,
                            x_train=x_train,        #X:    n_sequences/2 *(x_j: (n_steps * feature_size‘) )  (feature_size‘=feature_size*segment_width=24*21)
                            TC_train=TC_train,      #TC:   n_sequences/2 *(tc_j: (n_steps * TC_size’) )      (TC_size’=TC_size*segment_width=6*21)  (Tonal_centroids)
                            y_train=y_train,        #Y:    n_sequences/2 * n_steps
                            y_cc_train=y_cc_train,  #Ycc:  n_sequences/2 * n_steps    (chord change)
                            y_len_train=y_len_train,#Ylen: n_sequences/2              (记录了X_i[j]的长度即n_steps)
                            x_valid=x_valid,
                            TC_valid=TC_valid,
                            y_valid=y_valid,
...
if __name__ == '__main__':
    ...
    augment_Billboard() #移调以获得更多的训练数据
    segment_Billboard() #若干frame合并为一个segment->可用于FFN、CNN等
    reshape_Billboard() #若干segment合并为一个sequence->可用于TNN、LSTM、RNN等
