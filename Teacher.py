...
class Harmony_Transformer(object):
    ...
    def _get_relative_position(self, n_steps, n_units=512, max_dist=2, name='relative_position_encodings'):
        ...
        center = n_vectors // 2
        pos_enc = tf.get_variable(name, dtype=tf.float32, shape=[n_vectors, n_units], initializer=tf.keras.initializers.glorot_normal())
    ...
    def train(self,save_model=False,load_model=False,save_path="save/teacher/model.ckpt",load_path="save/teacher/model.ckpt",train=True,test=False,generate_soft_target=False, finnished_step=0):
        # load input data
        ...
        with tf.Session() as sess:
            ...
            if train:
                ...
                for step in range(self._training_steps):
                    ...
                    if step==0 and finnished_step!=0:
                        step=finnished_step
                        annealing_slope *= self._annealing_rate**np.floor(step/epoch)
                    ...
                    if step % (epoch // 2) == 0:
                        ...
                        if save_model:
                            saver_path = saver.save(sess, save_path)  # 将模型保存到save/model.ckpt文件
                            print("Model saved in file:", saver_path)

            if test:
                epoch = num_examples_train // self._batch_size
                end_slope = self._annealing_rate ** (self._training_steps / epoch)

                for i in range(1):
                    T1 = time.time()
                    test_run_list = [loss, loss_ct, loss_c, L2_regularizer, chord_change_predictions, chord_predictions, accuracy]
                    test_feed_fict = { x: x_valid,
                                       ...
                                       slope: end_slope,
                                       stochastic_tensor: True}
                    test_loss, test_loss_ct, test_loss_c, test_L2, test_cc_pred, test_c_pred, test_acc = sess.run(test_run_list, feed_dict=test_feed_fict)
                    T2 = time.time()
                    cost = T2 - T1
                    print("\n------ test: test_loss %.4f (ct %.4f, c %.4f, L2 %.4f), test_accuracy %.4f ------" % (test_loss, test_loss_ct, test_loss_c, test_L2, test_acc))
                    print("compute time: "+str(cost)+"s")


                    
            if generate_soft_target:
                epoch = num_examples_train // self._batch_size
                end_slope = self._annealing_rate**(self._training_steps/epoch)
                batch_new=200

                soft_target_test=None
                soft_target_train=None
                for i in range(np.ceil(x_train.shape[0] / batch_new).astype('int')):
                    train_run_list = [loss, loss_ct, loss_c, L2_regularizer, chord_change_predictions, chord_predictions, accuracy, logits]
                    train_feed_fict = {x: x_train[i*batch_new:min((i+1)*batch_new,x.shape[0])],
                                      y_cc: y_cc_train[i*batch_new:min((i+1)*batch_new,x.shape[0])],
                                      y: y_train[i*batch_new:min((i+1)*batch_new,x.shape[0])],
                                      y_len: y_len_train[i*batch_new:min((i+1)*batch_new,x.shape[0])],
                                      dropout_rate: self._dropout_rate,
                                      is_training: False,
                                      global_step: self._training_steps,
                                      slope: end_slope,
                                      stochastic_tensor: True}
                    train_loss, train_loss_ct, train_loss_c, train_L2, train_cc_pred, train_c_pred, train_acc, train_logits = sess.run(train_run_list, feed_dict=train_feed_fict)
                    temp=train_logits
                    if soft_target_train is None:
                        soft_target_train=temp
                    else:
                        soft_target_train=np.concatenate((soft_target_train,temp),axis=0)
                    #print(soft_target_train.shape)

                for i in range(np.ceil(x_valid.shape[0] / batch_new).astype('int')):
                    test_run_list = [loss, loss_ct, loss_c, L2_regularizer, chord_change_predictions, chord_predictions,accuracy, logits]
                    test_feed_fict = {x: x_valid[i*batch_new:min((i+1)*batch_new,x.shape[0])],
                                       y_cc: y_cc_valid[i*batch_new:min((i+1)*batch_new,x.shape[0])],
                                       y: y_valid[i*batch_new:min((i+1)*batch_new,x.shape[0])],
                                       y_len: y_len_valid[i*batch_new:min((i+1)*batch_new,x.shape[0])],
                                       dropout_rate: self._dropout_rate,
                                       is_training: False,
                                       global_step: self._training_steps,
                                       slope: end_slope,
                                       stochastic_tensor: True}
                    test_loss, test_loss_ct, test_loss_c, test_L2, test_cc_pred, test_c_pred, test_acc, test_logits = sess.run(test_run_list, feed_dict=test_feed_fict)
                    temp = test_logits
                    if soft_target_test is None:
                        soft_target_test=temp
                    else:
                        soft_target_test=np.concatenate((soft_target_test,temp),axis=0)
                    #print(soft_target_test.shape)

                #print(soft_target_train.shape)
                #print(soft_target_test.shape)
                outputdir = 'preprocessed_data/soft_target.npz'  # with segment
                with open(outputdir, "wb") as output_file:
                    np.savez_compressed(output_file,soft_target=soft_target_train,soft_target_test=soft_target_test) # soft_target: [train_size=1000, n_steps, n_classes]
                print("soft_target saved as"+outputdir)


if __name__ == '__main__':
    model = Harmony_Transformer()
    model.train(save_model=True,train=True,test=True,generate_soft_target=True)
    #model.train(save_model=False, train=False, test=False, generate_soft_target=True, load_model=True)
    #model.train(save_model=False, train=False, test=True, generate_soft_target=False, load_model=True)
